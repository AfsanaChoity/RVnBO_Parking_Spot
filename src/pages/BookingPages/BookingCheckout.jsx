...
export default function BookingCheckout() {
  ...
  // FIX: helper to normalize all dates to start-of-day (removes off-by-one)
  const norm = (d) => (d ? dayjs(d).startOf("day") : null); // FIX

  // Load from localStorage (normalize to startOf('day'))
  useEffect(() => {
  ...
  const handleCheckInChange = (date) => {
    if (!date) {
      setCheckInDate(null);
      localStorage.removeItem("checkIn");   // ✅ clear storage
      return;
    }
    const d = date.startOf("day"); // FIX
    setCheckInDate(d);
    localStorage.setItem("checkIn", date.format()); // format unchanged
    // (optional) ensure checkout >= checkin
    if (checkOutDate && d.isAfter(checkOutDate, "day")) {
      setCheckOutDate(d); // keep simple; or d.add(1,'day') if you want
      localStorage.setItem("checkOut", d.format());
    }
  };

  // Handle check-out date change (normalize, but keep storage format unchanged)
  const handleCheckOutChange = (date) => {
    if (!date) {
      setCheckOutDate(null);
      localStorage.removeItem("checkOut");  // ✅ clear storage
      return;
    }
    const d = date.startOf("day"); // FIX
    setCheckOutDate(d);
    localStorage.setItem("checkOut", date.format()); // format unchanged
  };

  // Recalculate nights (always use startOf('day'))
  useEffect(() => {
